...
// Manual invoice creation endpoint for staff — allows providing manualDueNumber to override auto-calculation
export async function POST(request: NextRequest) {
  try {
    const user = getUserFromRequest(request);
    if (!user || !(hasMinimumRole(user, 'admin') || hasMinimumRole(user, 'staff'))) {
